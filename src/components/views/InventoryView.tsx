...
export function InventoryView() {
  ...
  useEffect(() => { fetchInventory(); fetchVisibility(); }, [user]);
  ...
  const fetchInventory = async () => {
    if (!user) return;
    setLoading(true);
    const { data } = await supabase.from("user_inventory").select("*").eq("owner_id", user.id).order("acquired_at", { ascending: false });
  ...
  const toggleHide = async (itemId: string, currentlyHidden: boolean) => {
    ...
    toast({ title: currentlyHidden ? t("itemShown") : t("itemHidden") });
  };
  ...
  const giftItem = async (itemId: string) => {
    ...
    const { data, error } = await supabase.rpc("gift_item", { _item_id: itemId, _from_user: user.id, _to_username: cleanUsername });
    ...
    else { toast({ title: t("giftSent") }); setGiftingId(null); setGiftUsername(""); fetchInventory(); }
    setSending(false);
  };
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold flex items-center gap-2">
          <Package className="w-6 h-6 text-primary" /> {t("inventory")}
        </h2>
      ...
      {showSettings && (
        <GlassCard className="p-4 space-y-3">
          <h3 className="font-semibold text-sm">{t("inventoryVisibility")}</h3>
          <div className="flex gap-2">
            {[
              ...
              <button key={opt.value} onClick={() => updateVisibility(opt.value)}
                className={`flex-1 py-2 px-3 rounded-lg text-xs font-medium transition-colors border ${visibility === opt.value ? "bg-primary/20 border-primary/40 text-primary" : "border-border hover:bg-muted/30 text-muted-foreground"}`}>
                {opt.label}
              </button>
            ))}
          </div>
      ...
      {loading ? (
        ...
        <GlassCard className="text-center py-12">
          ...
          <h3 className="text-lg font-semibold mb-2">{t("inventoryEmptyShort")}</h3>
          <p className="text-muted-foreground">{t("buySkinOnMarket")}</p>
        </GlassCard>
      ) : (
            ...
            <GlassCard key={item.id} className={`overflow-hidden ${item.is_hidden ? "opacity-60" : ""}`}>
              ...
              <div className="p-4 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => toggleHide(item.id, item.is_hidden)} className="p-1.5 hover:bg-muted/50 rounded-lg transition-colors" title={item.is_hidden ? t("show") : t("hide")}>
                ...
                {giftingId === item.id ? (
                  <div className="space-y-2">
                    <FlameInput placeholder={t("recipientUsername")} value={giftUsername} onChange={(e) => setGiftUsername(e.target.value)} />
                    <div className="flex gap-2">
                      <FlameButton size="sm" className="flex-1" onClick={() => giftItem(item.id)} disabled={!giftUsername.trim() || sending}>
                        <Send className="w-3 h-3 mr-1" /> {sending ? "..." : t("send")}
                      </FlameButton>
                  ...
                  <FlameButton size="sm" variant="outline" className="w-full" onClick={() => setGiftingId(item.id)}>
                    <Gift className="w-3 h-3 mr-1" /> {t("giftBtn")}
                  </FlameButton>
                )}
              </div>
            </GlassCard>
