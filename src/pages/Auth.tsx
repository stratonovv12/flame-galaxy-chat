      ...
      if (isLogin) {
        ...
        if (error) {
          ...
          if (error.message.includes("Invalid login")) {
            ...
            toast({ title: "Email не подтверждён", description: "Проверьте почту и подтвердите регистрацию", variant: "destructive" });
          } else {
        ...
        if (error) {
          if (error.message.includes("already registered")) {
            toast({ title: "Ошибка регистрации", description: "Этот email уже зарегистрирован", variant: "destructive" });
          } else {
            toast({ title: "Ошибка", description: error.message, variant: "destructive" });
          }
        } else {
          toast({ title: "Успешная регистрация!", description: "Проверьте почту для подтверждения аккаунта" });
          setIsLogin(true);
        }
      }
    } finally {
    ...
    <div className="min-h-screen cosmic-bg flex items-center justify-center p-4">
      ...
      <GlassCard className="w-full max-w-md p-8 relative z-10">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-muted-foreground mt-2">{isLogin ? "Добро пожаловать!" : "Создайте аккаунт"}</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="relative">
            <FlameInput type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} className="pl-12" />
            <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
          </div>
          <div className="relative">
            <FlameInput type={showPassword ? "text" : "password"} placeholder="Пароль" value={password} onChange={e => setPassword(e.target.value)} className="pl-12 pr-12" />
